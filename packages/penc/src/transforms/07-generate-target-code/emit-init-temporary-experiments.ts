...
assert(PKG_DIR);
const STD_DIR = path.join(PKG_DIR, 'dist/temporary-experiments');
assert(fs.existsSync(STD_DIR));


// TODO: temp testing...
export function emitInitTemporaryExperiments(emit: Emitter) {
    emit.down(2).text(`function initTemporaryExperiments() {`).indent();
