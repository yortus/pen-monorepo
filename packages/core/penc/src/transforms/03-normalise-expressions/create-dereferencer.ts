...
/**
 * Returns a function that gets the node referred to by an Identifier node. Steps through references until either a
 * non-Identifier node is found, or a circular reference is detected.
 */
export function createDereferencer(lookup: (name: string) => V.Expression<300>) {

    // The dereference function, closed over the given AST.
    return function deref(id: V.Identifier) {
...
export type DereferenceFunction = ReturnType<typeof createDereferencer>;
