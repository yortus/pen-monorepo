import {allNodeKinds, Expression, GenericExpression, Identifier, MemberExpression, moduleFromBindingList} from '../../ast-nodes';
...
export function resolveSymbols(ast: AST): AST {
    ...
    function internalResolve({gen, arg, env}: {gen: GenericExpression, arg: Expression, env?: Scope}) {
        ...
        mapNode(topMod, rec => ({ // NB: top-level return value isn't needed, since everything has a symbol by then.
            Identifier: id => {
