...
export function createSymbolTable(options?: SymbolTableOptions) {
    ...
    const RESERVED_UNIQUE_NAMES = ['ꐚstart'];
    ...
    function createUniqueName(name: string): string {
        ...
        // Ensure no clashes with pen VM or JS buildins by adding a `ꐚ` prefix (NB: this is U+A41A, not # hash).
        let result = `ꐚ${name}`;
        let counter = 1;
        while (existingUniqueNames.has(result)) result = `ꐚ${name}ᱻ${++counter}`;
